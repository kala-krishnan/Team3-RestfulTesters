...
Feature: LMS Class Module
  ...
  Scenario Outline: Verify Update ClassRecording in Class module
    Given Admin creates PUT Request with "<requestType>" in Class module with request body
    When Admin sends PUT HTTPS Request update Class module with "<requestType>"
    Then Admin receives "<Status>" for Update Class module request

    Examples: 
      | requestType          | Status |
      | PutClassValidClassId |    200 |
